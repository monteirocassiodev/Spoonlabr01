...
import { OrgNode, AppState, AnalysisReport, Transaction } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (status === 'success' && token === KIWIFY_SECRET_TOKEN) {
      const transactions: Transaction[] = JSON.parse(localStorage.getItem('spoon_transactions') || '[]');
      const isRecent = transactions.some(t => t.timestamp > Date.now() - 30000); // Debounce
      
      if (!isRecent) {
        const newTx: Transaction = {
          id: 'KIW-' + Math.random().toString(36).substr(2, 6).toUpperCase(),
          timestamp: Date.now(),
          amount: PLAN_PRICE,
          method: 'KIWIFY',
          status: 'COMPLETED'
        };
        localStorage.setItem('spoon_transactions', JSON.stringify([...transactions, newTx]));
      }
      ...
      if (savedNode) {
        try {
          setRootNode(JSON.parse(savedNode));
  ...
  if (appState === AppState.ADMIN_PANEL) return <AdminDashboard onBack={() => setAppState(AppState.EDITING)} />;
        ...
        <div className="flex items-center gap-4">
          {isAutoUnlocked && (
            <div className="flex items-center gap-2 bg-emerald-50 text-emerald-600 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100 shadow-sm">
              <CheckCircle size={14} className="fill-emerald-500 text-white"/> Premium Unlocked
            </div>
          )}
          ...
          {isAdminMode && <button onClick={() => setAppState(AppState.ADMIN_PANEL)} className="px-4 py-2 bg-gray-900 text-white rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2"><LayoutGrid size={14} /> Analytics</button>}
      ...
      <main className="pt-24 pb-32 px-8">
        <div className="max-w-[1400px] mx-auto">
          <section className="mb-12 flex items-end justify-between">
            ...
            {!report && !isStreaming && <button onClick={() => { setRootNode(INITIAL_NODE); localStorage.removeItem('spoonlab_pending_node'); }} className="text-[9px] font-black text-gray-300 hover:text-red-500 uppercase flex items-center gap-2"><RefreshCw size={12} /> Resetar Projeto</button>}
          </section>
          ...
          <div className="relative">
             {report && !isStreaming && !isAutoUnlocked && (
               <div className="absolute inset-0 z-[40] bg-white/60 backdrop-blur-[4px] rounded-[3rem] flex items-center justify-center flex-col p-6">
                 <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-gray-100 flex flex-col items-center text-center max-w-md">
                    <div className="w-20 h-20 bg-blue-50 text-blue-600 rounded-3xl flex items-center justify-center mb-8 shadow-sm">
                      <Lock size={32} />
                    </div>
                    <h4 className="text-2xl font-black text-gray-900 mb-3 tracking-tighter">Blueprint Analisado</h4>
                    <p className="text-sm text-gray-500 mb-10 font-medium leading-relaxed">
                      Sua nova estrutura está pronta. Para visualizar as automações, o plano de 90 dias e o ROI projetado, libere sua licença Master.
                    </p>
                    <div className="flex flex-col w-full gap-4">
                      <button onClick={() => window.open(KIWIFY_CHECKOUT_URL, '_blank')} className="bg-blue-600 text-white w-full py-5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-blue-700 shadow-xl transition-all">Liberar Acesso Agora</button>
                      <button onClick={() => { setReport(null); setIsStreaming(false); }} className="text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-gray-900 py-2">Voltar ao Editor</button>
                    </div>
                 </div>
               </div>
             )}
             
             <div className="org-canvas bg-white border border-gray-100 rounded-[3rem] min-h-[700px] shadow-sm mb-20 overflow-auto flex justify-center p-24">
                <NodeEditor node={rootNode} onUpdate={setRootNode} isRoot={true} onAddParent={() => {}} onSetUserPosition={(id) => {}} />
             </div>
          </div>

          {!report && !isStreaming && (
            <div className="flex justify-center mt-12">
              <button onClick={startAnalysis} className="group flex items-center gap-6 bg-blue-600 hover:bg-blue-700 text-white px-16 py-8 rounded-[2rem] font-black text-2xl transition-all shadow-2xl shadow-blue-100 active:scale-95">
                <Sparkles size={32} /> Gerar Blueprint Estratégico <ChevronDown size={32} className="opacity-40" />
              </button>
            </div>
          )}
